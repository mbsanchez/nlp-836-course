...
public class IRSystem {
    ...
    ArrayList<LinkedHashSet<String>> fast_documents;
    ...
    void index() {
        ...
        /* End DONE */
        fast_documents = new ArrayList<LinkedHashSet<String>>();
        for(ArrayList<String> document: documents){
            fast_documents.add( new LinkedHashSet<String>( document ) );
        }
        
        for (String word : vocab) {
            for (int d = 0; d < fast_documents.size(); d++) {
                if (fast_documents.get(d).contains(word)) {
    ...
    void computeTFIDF() {
        /** DONE: Compute and store TF-IDF values for words and documents.
        ...
        for (int d = 0; d < documents.size(); d++) {
            for(String w : documents.get(d)){
                tfidf.incrementCount(w, d, 1);
            }
        }
        ...
        for (int d = 0; d < documents.size(); d++) {
            for (String word : vocab){
                Double count = tfidf.getCount(word, d);
                double tf = 0;
                if(count!=null && count>0)
                    tf = 1 + Math.log10 ( count );
                double idf = Math.log10 (  N / (double)invertedIndex.get(word).size() );
                tfidf.setCount(word, d, tf*idf);
            }
        }
    }
    ...
    double getTFIDF(String word, int doc) {
        ...
        /** DONE: Return the tf-idf weigthing for the given word (string) and
         * document index.
         */
        return tfidf.getCount(word, doc);
    }
    ...
    ArrayList<Integer> getPosting(String word) {
        ...
        /** DONE: Return the list of postings for a word. */
        /** End DONE */
        return posting;
    }
    ...
    ArrayList<Integer> booleanRetrieve(ArrayList<String> query) {
         ...
         * query).
         */
        /** DONE: Implement Boolean retrieval. You will want to use your
         * inverted index that you created in index().
         * Right now this just returns all the possible documents!
         * Return an empty list if the query does not return any documents.
         */
        ArrayList<Integer> docs = new ArrayList<Integer>();
        ArrayList<Integer> list1 = invertedIndex.get(query.get(0));
        
        for(int i=1;i<query.size();i++){
            ArrayList<Integer> list2 = invertedIndex.get(query.get(i));
            docs = ordListIntersect(list1, list2);
            list1 = docs;
        }
    ...
    public ArrayList<Integer> ordListIntersect(ArrayList<Integer> ordList1, ArrayList<Integer> ordList2){
        ArrayList<Integer> result = new ArrayList<Integer>();
        
        for(int p1=0, p2=0;p1<ordList1.size() && p2<ordList2.size();){
            if(ordList1.get(p1)==ordList2.get(p2)){
                result.add(ordList1.get(p1));
                p1++; p2++;
            }else if(ordList1.get(p1)<ordList2.get(p2))
                p1++;
            else
                p2++;
        }
    ...
    public double calculateTFIDFLenght(int d){
        double length=0;
        
        for(String word : vocab){
            Double wtd = tfidf.getCount(word, d);
            length += wtd * wtd;
        }
    ...
    PriorityQueue<Integer> rankRetrieve(ArrayList<String> query) {
        ...
        for(int d=0;d<documents.size();d++){
            double docLenght = calculateTFIDFLenght(d);
            for(String term : query){
                scores[d]+= tfidf.getCount(term, d) / docLenght;
            }
        }
